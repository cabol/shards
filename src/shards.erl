...
%% @doc Returns the partition PIDs associated with the given table `Tab'.
-spec partition_owners(Tab) -> [OwnerPid] when
      Tab      :: tab(),
      OwnerPid :: pid().
partition_owners(Tab) ->
  [Child || {_, Child, _, _} <- supervisor:which_children(Tab)].
